...
public:
    ...
    template <typename T> bool operator()(const T* const x1, const T* const x2, const T* const x3, T* e) const{
        ...
        for (uint i=0; i<edgeVertices.size(); i++)
        {
            ...
            if(index != -1)
            {
                T x = T(model->desiredNormals[index].x) - vertexNormals[index][0];
                T y = T(model->desiredNormals[index].y) - vertexNormals[index][1];
                T z = T(model->desiredNormals[index].z) - vertexNormals[index][2];
    ...
    template<typename T> void calcFaceNormal(T** faceNormals, T* v1, T* v2, const T* const x1, const T* const x2, const T* const x3, uint i) const
    {
        ...
        T vertex1[3];
        assign(vertex1, index, x1, x2, x3);

        index = edgeIndices[i][1];
        T vertex2[3];
        assign(vertex2, index, x1, x2, x3);

        index = edgeIndices[i][2];
        T vertex3[3];
        assign(vertex3, index, x1, x2, x3);
    ...
    template<typename T> void calcVertexNormal(T** vertexNormals, T** faceNormals, const T* const x1, const T* const x2, const T* const x3, uint index) const
    {
        ...
        for(uint i=0; i<edgeAdjacentFaces[index].size(); i++)
        {
            ...
            for(int vIndex=0; vIndex < 3; vIndex++)
            {
                ...
                T pos1[3];
                assign(pos1, indx, x1, x2, x3);


                T pos2[3];
                assign(pos2, index, x1, x2, x3);
            ...
            assign(edge1, indx, x1, x2, x3);

            indx = edgeIndices[edgeAdjacentFaces[index][i]][other1];
            assign(edge1Sub, indx, x1, x2, x3);


            indx = edgeIndices[edgeAdjacentFaces[index][i]][thisVertexIndex];
            assign(edge2, indx, x1, x2, x3);

            indx = edgeIndices[edgeAdjacentFaces[index][i]][other2];
            assign(edge2Sub, indx, x1, x2, x3);

            for(uint j=0; j<3; j++)
            {
                edge1[j] = edge1[j] - edge1Sub[j];
                edge2[j] = edge2[j] - edge2Sub[j];
            }

            normalize(edge1);
            normalize(edge2);

            T incidentAngle = angle(edge1, edge2);
...
void TargetOptimization::optimize(){
   ...
   CostFunction* cost_function_Eint =
        new AutoDiffCostFunction<CostFunctorEint, NORMALS, NORMALS, NORMALS, NORMALS>(new CostFunctorEint(model, EINT_WEIGHT));
   ...
   CostFunction* cost_function_Edir =
        new AutoDiffCostFunction<CostFunctorEdir, NORMALS, NORMALS, NORMALS, NORMALS>(new CostFunctorEdir(x_sources, EDIR_WEIGHT));
...
//    problem.AddResidualBlock(cost_function_Eint2, NULL, x1, x2, x3);
   ...
   // std::cout << summary.BriefReport() << std::endl;
    ...
    for (int i=0; i<NORMALS; i++)
    {
        model->meshes[0].faceVertices[i]->Position.x = x1[i];
        model->meshes[0].faceVertices[i]->Position.y = x2[i];
        model->meshes[0].faceVertices[i]->Position.z = x3[i];
    }
